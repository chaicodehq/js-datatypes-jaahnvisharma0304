...
export function generateReportCard(student) {
  ...
  for (const [subject, mark] of marksEntries) {
  ...
  const entries = Object.entries(student.marks).sort((a, b) => b[1] - a[1]);
  const highestSubject = entries[0][0];
  const lowestSubject = entries[entries.length - 1][0];
  
  // Find passed and failed subjects
  const passedSubjects = marksEntries.filter(([_, mark]) => mark >= 40).map(([subject]) => subject);
  const failedSubjects = marksEntries.filter(([_, mark]) => mark < 40).map(([subject]) => subject);
